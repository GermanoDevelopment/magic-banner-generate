  ...
  try {
    ...
    if (!eventoId || !foto) {
      return NextResponse.json(
        { erro: 'Campos eventoId e foto são obrigatórios.' },
        { status: 400 }
      );
    }
    ...
    if (!validMimeTypes.includes(foto.type)) {
      return NextResponse.json(
        {
          erro: 'Formato de arquivo inválido. Apenas JPG e PNG são permitidos.',
        },
        { status: 400 }
      );
    }
    ...
    if (foto.size > MAX_SIZE) {
      return NextResponse.json(
        { erro: 'O tamanho do arquivo excede o limite de 5MB.' },
        { status: 400 }
      );
    }
